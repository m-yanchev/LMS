...
import {FormDialog} from "../../../depricated/common/components/Dialog";
import {Problem as ProblemComponent} from "../../../depricated/common/components/Problem";
import {ButtonWithTooltip} from "../../../depricated/common/components/ButtonWithTooltip";
import {B} from "../../../depricated/common/components/Text";
...
const GETTING_SEND_SOLUTIONS_AVAILABLE_ERROR_MESSAGE = 'Возникла ошибка. ' +
    'Попробуйте обновить страницу.'
...
export function TestCardContent(props: TestCardContentProps): Element<typeof Fragment> {
    ...
    const {heading, fullTime, leftTime, highestEstimate, prevEstimate, isPending} = content
    const timeDesc = isPending ? {
        desc: "Общее время выполнения", value: leftTime, button: "Продолжить"
    } : fullTime > leftTime ? {
...
export function TestDialog(props: RunningDialogProps): Element<typeof FormDialog> {
    ...
    const {open, test, onCloseCall} = props
    ...
    if (isGettingProblemsError) errorMessage = GETTING_PROBLEMS_ERROR_MESSAGE
    if (isMakingSolutionError) errorMessage = MAKING_SOLUTION_ERROR_MESSAGE
    if (puttingProblemResultError) errorMessage = PUTTING_PROBLEMS_ERROR_MESSAGE

    const handleFail = () => {
        errorMessage = GETTING_SEND_SOLUTIONS_AVAILABLE_ERROR_MESSAGE
    }

    return <>
        {(loadingProblems || makingSolution) ? <LoadingProgress/> :
            <FormDialog open={open}
                        title={'Самостоятельная работа "' + test.heading + '"'}
                        warningMessage={errorMessage}
                        message={message}
                        actions={<DialogButtonGroup onClick={handleClick} test={test} onFail={handleFail}/>}
                        onClose={handleClose}>
